...
class ReorderableDragList<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Se a lista está vazia e há um widget de vazio, exibe ele
    if (items.isEmpty && emptyWidget != null) {
      return emptyWidget!;
    }

    // Se a lista está vazia e não há widget de vazio, retorna SizedBox vazio
    if (items.isEmpty) {
    ...
    return ReorderableListView.builder(
      ...
      physics: physics,
      padding: padding,
      buildDefaultDragHandles: false, // Sempre false para usar handles customizados
      itemCount: items.length,
      onReorder: enabled ? onReorder : (oldIndex, newIndex) {}, // Desabilita se enabled = false
...
class _DragListItem<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        // Drag handle (só aparece se enabled = true)
        if (enabled)
          Padding(
