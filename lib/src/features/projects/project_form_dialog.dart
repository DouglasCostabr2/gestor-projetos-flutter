...
class ProjectFormDialog extends StatefulWidget {
  ...
  const ProjectFormDialog(
      {super.key, this.initial, this.fixedClientId, this.fixedCompanyId});
...
class _ProjectFormDialogState extends State<ProjectFormDialog> {
  ...
  final _description =
      TextEditingController(); // Controller para descrição com menções
  String _descriptionText = ''; // Texto da descrição (plain text)
  String _descriptionJson =
      ''; // JSON da descrição (rich text - AppFlowy Editor)
  ...
  void initState() {
    ...
    _descriptionJson =
        i != null ? (i['description_json'] ?? '').toString() : '';
    _descriptionText = i != null ? (i['description'] ?? '').toString() : '';

    if (i != null) {
      _name.text = (i['name'] ?? '').toString();
      _description.text =
          _descriptionText; // Inicializar controller com texto da descrição
      ...
      if (cur != null && ['BRL', 'USD', 'EUR'].contains(cur)) {
        _currencyCode = cur;
      }
  ...
  Future<void> _loadCatalogItems(String projectId) async {
    try {
      ...
      final rows = await client
          ...
          .select(
              'kind, item_id, name, currency_code, unit_price_cents, quantity, position, comment')
          .eq('project_id', projectId)
      ...
      for (final pid in productIds) {
        try {
          final p = await client
              ...
              .maybeSingle();
          if (p != null) {
            final url = (p['image_thumb_url'] as String?) ??
  ...
  Future<void> _loadPackageProductsFor(String packageId) async {
    try {
      final rows = await Supabase.instance.client
          .from('package_items')
          .select(
              'quantity, comment, position, products:product_id(id, name, image_url, image_thumb_url)')
      ...
      for (final r in rows as List<dynamic>) {
        ...
        list.add({
          ...
          'thumbUrl': (prod?['image_thumb_url'] as String?) ??
      ...
      setState(() {
        _packageProductsById[packageId] = list;
      });
    } catch (e) {
      setState(() {
        _packageProductsById[packageId] = const [];
      });
    }
  }
  ...
  Future<void> _repriceCatalogItemsForCurrency(String currency) async {
    try {
      final client = Supabase.instance.client;
      final productIds = _catalogItems
          .where((e) => e.itemType == 'product')
          .map((e) => e.itemId)
          .toSet()
          .toList();
      final packageIds = _catalogItems
      ...
      setState(() {
        _catalogItems.asMap().forEach((idx, it) {
          ...
          _catalogItems[idx] =
              it.copyWith(priceCents: newPrice, currency: currency);
        });
      });
    } catch (e) {
      // Ignorar erro (operação não crítica)
  ...
  int get _catalogSumCents => _catalogItems
      .where((it) => it.currency == _currencyCode)
      .fold<int>(0, (sum, it) => sum + it.priceCents);
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final base = <String, dynamic>{
        ...
        'description_json':
            descJson.isEmpty ? null : descJson, // Salvar JSON do rich text
      ...
      if (projectId.isNotEmpty) {
        ...
        if (_catalogItems.isNotEmpty) {
          final rows = _catalogItems.asMap().entries.map((e) {
            final idx = e.key;
            final it = e.value;
            return {
              'project_id': projectId,
              ...
              'quantity': 1,
              'comment': (it.comment?.trim().isEmpty ?? true)
                  ? null
                  : it.comment!.trim(),
              'position': idx,
              if (uid != null) 'created_by': uid,
          ...
          try {
            ...
            final looksLikeMissingColumn = msg.contains('comment') ||
                msg.contains('column') ||
          ...
          final processedPackageIds = <String>{};
          for (final pkg
              in _catalogItems.where((e) => e.itemType == 'package')) {
            if (processedPackageIds.contains(pkg.itemId)) continue;
            processedPackageIds.add(pkg.itemId);
          ...
          if (ppic.isNotEmpty) {
            try {
              await client.from('project_package_item_comments').upsert(ppic,
                  onConflict: 'project_id,package_id,product_id,position');
            } catch (e) {
              final msg = e.toString();
              final looksMissingPosition = msg.contains('position') ||
                  msg.contains('column') ||
                  msg.contains('PGRST204');
              try {
                ...
                if (looksMissingPosition) {
                  ...
                  try {
                    await client.from('project_package_item_comments').upsert(
                        ppicNoPos,
                        onConflict: 'project_id,package_id,product_id');
                  } catch (_) {
                    // Nova tentativa: replace manual
                    final pkgIds = ppicNoPos
                        .map((r) => (r['package_id'] ?? '').toString())
                        .toSet();
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Erro ao salvar projeto: ${e.toString()}')));
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    final canEditFinancial =
        appState.isAdmin || appState.isFinanceiro; // controla edição
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
              ...
              AsyncDropdownField<String>(
                ...
                loadItems: () async {
                  final rows = await Supabase.instance.client
                      ...
                      .order('name');
                  return (rows as List)
                      .map((item) => SearchableDropdownItem(
                  ...
                  _DashedActionBox(
                    onTap: () async {
                      final selected = await showDialog<_CatalogItem>(
                        context: context,
                        builder: (_) =>
                            _SelectCatalogItemDialog(currency: _currencyCode),
                      );
                      if (selected != null) {
                        setState(() {
                          _catalogItems.add(selected);
                          // Sempre atualizar o valor do projeto
                          _valueText.text = _formatCents(_autoTotalCents)
                              .replaceAll('.', ',');
                        });
                      }
                    },
                          ...
                          child: Column(
                              ...
                              children: [
                                    ...
                                    onPressed: () async {
                                      final text = await showDialog<String>(
                                        ...
                                        builder: (_) {
                                          ...
                                          return StandardDialog(
                                            title:
                                                'Comentário (${it.itemType == 'product' ? 'Produto' : 'Pacote'})',
                                            ...
                                            child: TextField(
                                              ...
                                              decoration: const InputDecoration(
                                                  hintText:
                                                      'Digite um comentário para este projeto'),
                                            ),
                                          );
                                        },
                                      );
                                ...
                                if (it.itemType == 'package') ...[
                                  ...
                                  Builder(builder: (_) {
                                    final kids =
                                        _packageProductsById[it.itemId];
                                    if (kids == null) {
                                                  ...
                                                  child: Column(
                                                      ...
                                                      children: [
                                                        Row(children: [
                                                          ...
                                                          IconOnlyButton(
                                                            tooltip:
                                                                'Editar comentário do produto do pacote',
                                                            icon: Icons
                                                                .edit_note_outlined,
                                                            iconSize: 18,
                                                            onPressed:
                                                              ...
                                                              final text =
                                                                ...
                                                                builder: (_) {
                                                                  ...
                                                                  return StandardDialog(
                                                                    title:
                                                                        'Comentário do produto (pacote)',
                                                                    width: StandardDialog
                                                                        .widthSmall,
                                                                    height: StandardDialog
                                                                        .heightSmall,
                                                                    actions: [
                                                                      TextOnlyButton(
                                                                    ...
                                                                    child: TextField(
                                                                        ...
                                                                        maxLines:
                                                                            4,
                                                                        decoration:
                                                                            const InputDecoration(hintText: 'Comentário deste projeto')),
                                                                  );
                                                                },
                                                              );
                                                              if (text !=
                ...
                IconTextButton(
                  onPressed: () {
                    setState(() {
                      ...
                      // Sempre atualizar o valor do projeto
                      _valueText.text =
                          _formatCents(_autoTotalCents).replaceAll('.', ',');
                    });
                  },
                  icon: Icons.add,
            ...
            Column(
                children: _costs.asMap().entries.map((e) {
              final i = e.key;
              final c = e.value;
              return Padding(
                padding: const EdgeInsets.symmetric(vertical: 6),
                  ...
                  SizedBox(
                    ...
                    child: GenericDropdownField<String>(
                      ...
                      items: [
                        ...
                        DropdownItem(
                            value: 'fixed',
                            label: _currencySymbol(_currencyCode)),
                      ],
            ...
            const SizedBox(height: 8),
            Column(
                children: _discounts.asMap().entries.map((e) {
              final i = e.key;
              final d = e.value;
              return Padding(
                padding: const EdgeInsets.symmetric(vertical: 6),
  ...
  List<Widget> _buildActions(bool canEditFinancial) {
    ...
    final buttonsRow = [
      ...
      GenericDropdownField<String>(
        value: ['BRL', 'USD', 'EUR'].contains(_currencyCode)
            ? _currencyCode
            : 'BRL',
        items: const [
        ...
        child: Column(
          ...
          children: [
            GenericNumberField(
              ...
              enabled:
                  true, // Habilitado para manter texto branco, mas será recalculado automaticamente
              allowDecimals: true,
...
class _CatalogItem {
  ...
  _CatalogItem({
    ...
    this.comment,
            ...
            '${DateTime.now().millisecondsSinceEpoch}_${itemType}_$itemId',
...
class _SelectCatalogItemDialog extends StatefulWidget {
  ...
  @override
  State<_SelectCatalogItemDialog> createState() =>
      _SelectCatalogItemDialogState();
}
...
class _SelectCatalogItemDialogState extends State<_SelectCatalogItemDialog> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final prods = await client
          .from('products')
          .select(
              'id, name, currency_code, price_cents, price_map, image_url, image_thumb_url, image_drive_file_id')
      ...
      if (!mounted) return;
      setState(() {
        _error = 'Falha ao carregar catálogo';
        _loading = false;
      });
    }
  }
  ...
  Widget _leadingThumb(Map<String, dynamic> p) {
    ...
    if (url == null || url.isEmpty) {
      return const CircleAvatar(
          radius: 18, child: Icon(Icons.image_not_supported, size: 18));
    }
    return ClipRRect(
      borderRadius: BorderRadius.circular(6),
  ...
  Widget build(BuildContext context) {
    return StandardDialog(
      ...
      actions: [
        TextOnlyButton(
            onPressed: () => Navigator.pop(context), label: 'Cancelar'),
      ],
      child: _loading
                      ...
                      itemBuilder: (context, i) {
                        ...
                        return ListTile(
                          ...
                          subtitle: Builder(builder: (_) {
                            final pm = (p['price_map'] as Map?)
                                ?.cast<String, dynamic>();
                            final cents =
                                pm != null && pm[widget.currency] is int
                                    ? pm[widget.currency] as int
                                    : ((p['currency_code'] == widget.currency)
                                        ? (p['price_cents'] as int? ?? 0)
                                        : 0);
                            return Text(
                                '${_sym(widget.currency)} ${_fmt(cents)}');
                          }),
                          onTap: () {
                            final pm = (p['price_map'] as Map?)
                                ?.cast<String, dynamic>();
                            ...
                            Navigator.pop(
                                ...
                                _CatalogItem(
                                  ...
                                  priceCents: cents,
                                  thumbUrl: (p['image_thumb_url'] as String?) ??
                                      (p['image_url'] as String?),
                                ));
                          },
                        );
