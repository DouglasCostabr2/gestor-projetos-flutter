...
/// Tab de configurações da organização (dados fiscais da empresa do usuário)
class OrganizationSettingsTab extends StatefulWidget {
  const OrganizationSettingsTab({super.key});

  @override
  State<OrganizationSettingsTab> createState() => _OrganizationSettingsTabState();
}

class _OrganizationSettingsTabState extends State<OrganizationSettingsTab> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final data = {
        ...
        'bank_branch': _bankBranch.text.trim().isEmpty ? null : _bankBranch.text.trim(),
      ...
      if (mounted) {
        ...
        await _loadOrganizationSettings(); // Reload to get the ID if it was created
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Dados fiscais e informações da sua empresa para emissão de invoices',
