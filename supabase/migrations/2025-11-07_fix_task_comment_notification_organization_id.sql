-- ============================================================================
-- Migration: Fix notify_task_comment to include organization_id
-- Date: 2025-11-07
-- Description: Updates notify_task_comment function to pass organization_id
--              to create_notification, fixing the NOT NULL constraint violation
-- ============================================================================
...
BEGIN
  ...
  SELECT id, title, assigned_to, assignee_user_ids, created_by, organization_id
  INTO v_task
  FROM public.tasks
  ...
  IF v_task.assigned_to IS NOT NULL AND v_task.assigned_to != NEW.user_id THEN
    -- Verificar se o responsável foi mencionado
    SELECT EXISTS (
...
COMMENT ON FUNCTION public.notify_task_comment() IS 'Cria notificação quando há novo comentário em tarefa (evita duplicatas com menções) - com organization_id';

-- Completion notice
DO $$
BEGIN
  RAISE NOTICE '✅ Fixed notify_task_comment function to include organization_id';
END $$;
